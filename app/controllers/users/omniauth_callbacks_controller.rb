  ...
  def facebook
    ...
    if uid != '' and provider != ''
      ...
      if !auth.nil?
        ...
        user = User.new(:provider => provider, :uid => uid, :name => name, :email => email, :fname => first_name, :lname => last_name, :token => access_token.credentials.token, :password => Devise.friendly_token[0,20])                        
        #raise user.inspect
        if user.save!
